...
const LABELS = { receipt: "Receipt upload", code: "Entry code", qr: "QR scan", barcode: "Barcode scan" };
...
export function CompetitionCard({ competition, featured = false }: { competition: Competition; featured?: boolean }) {
  ...
  return (
    <article className="group relative flex h-full flex-col overflow-hidden rounded-lg border border-rule bg-surface u-lift">
      <div className={featured ? "relative aspect-[16/8] overflow-hidden" : "relative aspect-[16/10] overflow-hidden"}>
        ...
        <span className="absolute left-3 top-3 inline-flex items-center gap-1.5 rounded-sm bg-white px-2.5 py-1.5 text-[0.72rem] font-semibold text-ink shadow-[0_1px_3px_rgba(20,21,28,0.12)]">
      ...
      <div className="flex flex-1 flex-col p-5">
        <div className="flex items-center gap-2">
          ...
          <span className="ml-auto u-label text-primary-ink">Competition</span>
        </div>
        ...
        <p className="mt-0.5 text-[0.95rem] font-semibold leading-snug text-ink">{competition.prize}</p>
        ...
        <div className="mt-auto pt-4">
          <Link
            ...
            className="inline-flex min-h-11 w-full items-center justify-between rounded-md bg-ink px-4 py-2.5 text-[0.85rem] font-semibold text-paper transition-colors hover:bg-primary"
          >
            {en.platform.competitions.view}
