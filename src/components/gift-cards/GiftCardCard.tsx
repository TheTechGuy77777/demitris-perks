    ...
    <article className="flex h-full flex-col overflow-hidden rounded-md border border-rule bg-surface u-lift">
      ...
      <div className="flex flex-1 flex-col p-4">
        <div className="flex items-center gap-2">
          ...
          <span className="ml-auto u-label text-ink-3">Gift card</span>
        </div>
        <Link href={`/gift-cards/${giftCard.slug}`} className="mt-3 rounded-sm">
          <h3 className="text-[1rem] font-semibold leading-snug tracking-[-0.015em] text-ink hover:text-primary">{giftCard.title}</h3>
        </Link>
